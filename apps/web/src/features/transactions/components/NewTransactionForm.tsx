...
function FormField({
  ...
  return (
    <div className="mb-4">
      <label className="block mb-1 font-medium" htmlFor={field.name}>{label}</label>
      <Input
        id={field.name}
        type={type}
      ...
      {field.state.meta.isTouched && field.state.meta.errors && (
        <span className="text-sm text-red-600">{field.state.meta.errors}</span>
      )}
    </div>
  )
}
...
export function NewTransactionForm() {
  ...
  const [developerProjectSearch, setDeveloperProjectSearch] = React.useState('')
  const [isProjectDropdownOpen, setIsProjectDropdownOpen] = React.useState(false)
  // State for property type suggestions dropdown
  const [showPropertyTypeSuggestions, setShowPropertyTypeSuggestions] = React.useState(false)
  // Local state for co-broking to ensure immediate UI updates
  const [localCoBrokingEnabled, setLocalCoBrokingEnabled] = React.useState(false)
  const [localCoBrokingDirection, setLocalCoBrokingDirection] = React.useState<'seller' | 'buyer'>('seller')
  ...
  React.useEffect(() => {
    ...
    if (form.state.values.coBrokingDirection === 'seller' || form.state.values.coBrokingDirection === 'buyer') {
      setLocalCoBrokingDirection(form.state.values.coBrokingDirection)
    }
  }, [form.state.values.coBrokingEnabled, form.state.values.coBrokingDirection])
  
  // Step content renderers
  // Local UI state for toggling secondary client fields
  const [showClientSecondaryFields, setShowClientSecondaryFields] = useState(false)
  ...
  function renderStep() {
    switch (step) {
      case 0:
        return (
          <>
            <form.Field name="marketType" validators={{
              onChange: value => !value ? 'Please select a market type.' : undefined
            }}>
              {field => (
                <div className="mb-4">
                  <label className="block mb-1 font-medium" id="market-type-label">Market Type</label>
                  <ToggleGroup
                    type="single"
                    value={field.state.value || ''}
                    onValueChange={val => {
                      if (val === 'Primary' || val === 'Secondary') {
                        ...
                        setFormErrors(prev => ({ ...prev, marketType: false }))
                      }
                    }}
                    ...
                    <ToggleGroupItem
                      ...
                      className={
                        field.state.value === 'Primary'
                          ? 'px-4 py-2 rounded bg-blue-600 text-white font-semibold shadow focus:outline-none focus:ring-2 focus:ring-blue-400'
                          : 'px-4 py-2 rounded bg-muted text-foreground hover:bg-blue-100 focus:outline-none focus:ring-2 focus:ring-blue-400'
                      }
                  ...
                  {(field.state.meta.isTouched && field.state.meta.errors || formErrors.marketType) && (
                    <span className="text-sm text-red-600 block mt-2">{formErrors.marketType ? 'Please select a market type.' : Array.isArray(field.state.meta.errors) ? field.state.meta.errors[0] : field.state.meta.errors}</span>
                  )}
                </div>
              )}
            </form.Field>
            {form.state.values.marketType === 'Primary' && (
              <form.Field name="developerProject" validators={{
                onChange: value => !value ? 'Please select a developer project.' : undefined
              }}>
                {field => {
                  const projects = [
                    ...
                    ...Array.from({ length: 30 }, (_, i) => ({ name: `Project ${i + 1}`, location: i % 2 === 0 ? 'KL' : 'Penang' }))
                  ]
                  const filtered = projects.filter(p => p.name.toLowerCase().includes(developerProjectSearch.toLowerCase()))
                  ...
                  return (
                    <div className="mb-4 relative">
                      <label className="block mb-1 font-medium">Developer Project</label>
                      <label htmlFor="developer-project-search" className="sr-only">Search projects</label>
                      <input
                        ...
                        onBlur={e => {
                          ...
                          setTimeout(() => {
                            ...
                            if (!field.state.value) setDeveloperProjectSearch('')
                      ...
                      {developerProjectSearch && filtered.length > 0 && isProjectDropdownOpen && (
                        <div className="absolute z-20 w-full bg-card border rounded shadow max-h-60 overflow-auto">
                          {Object.entries(grouped).map(([loc, projs]) => (
                            <div key={loc}>
                              <div className="px-2 pt-2 pb-1 text-xs font-semibold text-muted-foreground">{loc}</div>
                              {projs.slice(0, 20).map(p => (
                                <div
                                  key={p.name}
                                  className={`px-4 py-2 cursor-pointer hover:bg-blue-100 ${field.state.value === p.name ? 'bg-blue-50 font-semibold' : ''}`}
                                  tabIndex={0}
                                  role="option"
                                  ...
                                  onMouseDown={e => {
                                    ...
                                    // Clear any previous errors for this field
                                    setFormErrors(prev => ({ ...prev, developerProject: false }))
                      ...
                      {developerProjectSearch && filtered.length === 0 && isProjectDropdownOpen && (
                        <div className="absolute z-20 w-full bg-card border rounded shadow p-2 text-sm text-muted-foreground">No projects found.</div>
                      )}
                      {(field.state.meta.isTouched && field.state.meta.errors || formErrors.developerProject) && (
                        <span className="text-sm text-red-600">{formErrors.developerProject ? 'Please select a developer project.' : Array.isArray(field.state.meta.errors) ? field.state.meta.errors[0] : field.state.meta.errors}</span>
                      )}
                    </div>
                  )
                }}
            ...
            {form.state.values.marketType === 'Secondary' && (
              <form.Field name="secondaryType" validators={{
                onChange: value => !value ? 'Please select a secondary transaction type.' : undefined
              }}>
                {field => (
                  <div className="mb-4">
                    <label className="block mb-1 font-medium">Secondary Transaction Type</label>
                    <ToggleGroup
                      type="single"
                      value={field.state.value}
                      onValueChange={(val: string) => {
                        if (val) {
                          field.handleChange(val)
                          // Clear any previous errors for this field
                          setFormErrors(prev => ({ ...prev, secondaryType: false }))
                        }
                      }}
                      className="gap-2"
                    >
                    ...
                    {(field.state.meta.isTouched && field.state.meta.errors || formErrors.secondaryType) && (
                      <span className="text-sm text-red-600">{formErrors.secondaryType ? 'Please select a secondary transaction type.' : Array.isArray(field.state.meta.errors) ? field.state.meta.errors[0] : field.state.meta.errors}</span>
                    )}
                  </div>
                )}
              </form.Field>
            )}
            <form.Field name="transactionDate" validators={{
              onChange: value => !value ? 'Please select a transaction date.' : undefined
            }}>
              {field => (
                <div className="mb-4">
                  <label className="block mb-1 font-medium" htmlFor="transactionDate-picker">Transaction Date</label>
                  <Popover>
                    ...
                    <PopoverContent className="w-auto p-0" align="start">
                      <Calendar
                        ...
                        onSelect={date => field.handleChange(date ? date.toISOString().slice(0, 10) : '')}
                        initialFocus
                      />
                    </PopoverContent>
                  </Popover>
                  {field.state.meta.isTouched && field.state.meta.errors && (
                    <span className="text-sm text-red-600">{Array.isArray(field.state.meta.errors) ? field.state.meta.errors[0] : field.state.meta.errors}</span>
                  )}
                </div>
              )}
            </form.Field>
      ...
      case 1:
        return (
          <>
            ...
            {(form.state.values.marketType === 'Primary' || 
              (form.state.values.marketType === 'Secondary' && form.state.values.transactionType === 'Sale')) ? (
              <form.Field 
                name="totalPrice"
                validators={{
                ...
                {(field: AnyFieldApi) => {
                  ...
                  return (
                    <div className="space-y-2 mb-6">
                      <label className="block text-sm font-medium text-gray-700" htmlFor="primary-price">
                        {form.state.values.marketType === 'Primary' ? 'Transaction Price' : 'Total Price'} <span className="text-red-500">*</span>
                      </label>
                      <div className="relative rounded-md shadow-sm">
                        ...
                        <input
                          id="primary-price"
                          type="number"
                          className="block w-full pl-10 pr-12 sm:text-sm border-gray-300 rounded-md p-2 border focus:ring-blue-500 focus:border-blue-500"
                          placeholder="Enter price"
                          value={fieldValue}
                          onChange={e => field.handleChange(e.target.value)}
                          onBlur={field.handleBlur}
                        />
                        <span className="absolute inset-y-0 right-0 pr-3 flex items-center text-gray-500 sm:text-sm">MYR</span>
                      </div>
                      {field.state.meta.isTouched && field.state.meta.errors && (
                        <span className="text-sm text-red-600">{Array.isArray(field.state.meta.errors) ? field.state.meta.errors[0] : field.state.meta.errors}</span>
                      )}
              ...
              <form.Field 
                ...
                validators={{
                  onChange: (input: string | { value: string }) => {
                    ...
                    if (!value || value === '0') return 'This field is required'
                    const numValue = Number(value)
                    if (isNaN(numValue) || numValue <= 0) return 'Rent must be greater than 0'
                    return undefined
                  }
                }}
              >
                {(field: AnyFieldApi) => {
                  ...
                  return (
                    <div className="space-y-2 mb-6">
                      <label className="block text-sm font-medium text-gray-700" htmlFor="secondary-rent">
                      ...
                      <div className="relative rounded-md shadow-sm">
                        ...
                        <input
                          ...
                          type="text"
                          className="focus:ring-blue-500 focus:border-blue-500 block w-full pl-14 pr-12 sm:text-sm border-gray-300 rounded-md p-2 border"
                          placeholder="0"
          ...
          <>
            ...
            <div className="mb-4 pb-4 border-b border-gray-200">
              ...
              {(field: AnyFieldApi) => {
                ...
                const suggestions = [
                  'Bungalow', 'Semi-D', 'Condo', 'Double Storey Terrace', 'Apartment', 'Townhouse', 'Serviced Residence'
                ]
                const inputValue = field.state.value?.toLowerCase() || ''
                const filteredSuggestions = suggestions.filter(s => s.toLowerCase().includes(inputValue) && inputValue)
                return (
                  <div className="mb-4 relative">
                    <label className="block mb-1 font-medium" htmlFor="property-type-field">
                      Property Type
                      <span className="ml-2 cursor-pointer" tabIndex={0}>
                        <span title="Recommended: e.g. Bungalow, Semi-D, Condo, Double Storey Terrace" className="text-gray-400">&#9432;</span>
                      </span>
                    </label>
                    <input
                      ...
                      onBlur={() => {
                        ...
                        setTimeout(() => setShowPropertyTypeSuggestions(false), 200)
                        field.handleBlur()
                      }}
                    />
                    {/* Auto-suggestion dropdown */}
                    {filteredSuggestions.length > 0 && showPropertyTypeSuggestions && (
            ...
            <div className="mb-4 pb-4 border-b border-gray-200">
              ...
              <form.Field name="clientName" validators={{
                onChange: (value: string) => !value ? 'Client name is required' : undefined
              }}>
                {(field: AnyFieldApi) => (
              ...
              <form.Field name="clientPhone" validators={{
              ...
              <form.Field name="clientEmail" validators={{
                onChange: (value: string) => {
              ...
              <div className="mb-4">
                <label className="flex items-center gap-2 cursor-pointer">
                  <input
                    ...
                    onChange={e => setShowClientSecondaryFields(e.target.checked)}
              ...
              {showClientSecondaryFields && (
                <>
                  ...
                  <form.Field name="clientAcquisitionSource">
                    {(field: AnyFieldApi) => (
                      <div className="mb-4">
                        <label className="block mb-1 font-medium" htmlFor="client-acquisition-source">Client Acquisition Source</label>
                        <select
            ...
            <div className="mb-4 pb-4 border-b border-gray-200">
              ...
              <form.Field name="paymentMethod">
                {(field: AnyFieldApi) => (
                  <div className="mb-4">
                    <label className="block mb-1 font-medium" htmlFor="payment-method">Payment Method</label>
                    <select
                      id="payment-method"
                      value={field.state.value}
                      onChange={e => field.handleChange(e.target.value)}
      ...
      case 3:
        return (
          <>
            <div className="mb-4 pb-4 border-b border-gray-200">
              <h3 className="text-lg font-medium mb-3">Property Ownership</h3>
              <form.Field name="ownerName" validators={{onChange: (v: string) => !v ? 'Owner Name is required' : undefined}}>
                {(field: AnyFieldApi) => (
            ...
            <div className="mb-4 pb-4 border-b border-gray-200">
              <h3 className="text-lg font-medium mb-3">Co-Broking Information</h3>
              {/* Co-Broking Toggle */}
              <div className="mb-4">
                <label className="flex items-center gap-2 cursor-pointer">
                  <input
              ...
              {localCoBrokingEnabled && (
                <div className="pl-4 border-l-2 border-blue-100">
                  ...
                  <form.Field name="coBrokingAgentRen">
                    {(field: AnyFieldApi) => (
                      <FormField label="Co-Broking Agent REN Number" field={field} />
                    )}
                  </form.Field>
                  ...
                  <form.Field name="coBrokingAgentContact" validators={{
                    onChange: (value: any) => {
                    ...
                    {(field: AnyFieldApi) => (
                      <FormField label="Co-Broking Agent Contact Number" field={field} />
                    )}
                  </form.Field>
                  {/* Co-Broking Direction Radio */}
                  <div className="mb-4">
                    <label className="block mb-1 font-medium">Co-Broking Direction</label>
        ...
        return (
          <>
            ...
            <div className="mb-6 text-muted-foreground italic">
              Commission summary will be shown here once commission logic is implemented.
            </div>
            ...
            <form.Field name="commissionValue">
              {(field: AnyFieldApi) => (
                <FormField label="Commission Value" field={field} type="number" />
              )}
            </form.Field>
            ...
            <form.Field name="commissionPercentage">
              {(field: AnyFieldApi) => (
                <FormField label="Commission Percentage" field={field} type="number" />
              )}
            </form.Field>
          </>
        )
      case 5:
        return (
          <>
            <div className="mb-4">
              <label className="block mb-1 font-medium">Upload Documents (not yet implemented)</label>
      ...
      case 6:
        return (
          <>
            ...
            <pre className="bg-muted p-2 rounded text-xs mb-2">{JSON.stringify(form.state.values, null, 2)}</pre>
  ...
  return (
    <form
      className="p-6 border rounded-lg shadow-sm bg-card text-card-foreground max-w-xl mx-auto"
      onSubmit={form.handleSubmit}
    >
      <h2 className="mb-4 text-xl font-semibold">Transaction Form - Step {step + 1} of {STEPS.length}</h2>
      <div className="mb-2 text-lg font-medium">{STEPS[step]}</div>
      {renderStep()}
      <div className="flex gap-2 mt-6">
        {step > 0 && (
          <button type="button" className="px-4 py-2 rounded bg-gray-200" onClick={() => setStep(s => s - 1)}>
            Back
          </button>
        )}
        ...
        {step === 0 && (
          <button
            type="button"
            className={`px-4 py-2 rounded font-semibold ${form.state.values.marketType ? 'bg-blue-600 text-white hover:bg-blue-700' : 'bg-gray-300 text-gray-500 cursor-not-allowed'}`}
            onClick={() => {
              ...
              if (form.state.values.marketType === 'Primary' && !form.state.values.developerProject) {
                // Set error for developer project
                setFormErrors(prev => ({ ...prev, developerProject: true }))
                return
              }
              
              if (form.state.values.marketType === 'Secondary' && !form.state.values.secondaryType) {
            ...
            disabled={!form.state.values.marketType || (form.state.values.marketType === 'Primary' && !form.state.values.developerProject) || (form.state.values.marketType === 'Secondary' && !form.state.values.secondaryType)}
            aria-disabled={!form.state.values.marketType || (form.state.values.marketType === 'Primary' && !form.state.values.developerProject) || (form.state.values.marketType === 'Secondary' && !form.state.values.secondaryType) ? true : undefined}
          >
            Next
          </button>
        )}
        ...
        {step > 0 && step < STEPS.length - 1 && (
          <button type="button" className="px-4 py-2 rounded bg-blue-600 text-white" onClick={() => setStep(s => s + 1)}>
            Next
          </button>
        )}
        {step === STEPS.length - 1 && (
